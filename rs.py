...
class Line:
    def __init__(self, point1, point2):
        """For line formula y(x) = k * x + b, calc k and b params If the line is vertical, set "vertical" attr to True
        and save "x" position of the line.
        """
        self.point1 = point1
        self.point2 = point2
        self.vertical = False
...
class LabelUnwrapper:
    ...
    def load_points(self):
        ...
        (self.point_a, self.point_b, self.point_c, self.point_d, self.point_e, self.point_f) = self.points
    ...
    def unwrap_label_perspective(self, source_map):
        ...
        for row_index in range(self.ROW_COUNT - 1):
            for col_index in range(self.COL_COUNT - 1):
                ...
                self.dst_image[y_offset : y_offset + dy_int, x_offset : x_offset + dx_int] = dst
    ...
    def get_roi(self, image, points):
        ...
        return image[np.floor(rect[0][1]) : np.ceil(rect[2][1]), np.floor(rect[0][0]) : np.ceil(rect[1][0])]

    def calc_source_map(self):
        top_points = self.calc_ellipse_points(self.point_a, self.point_b, self.point_c, self.COL_COUNT)
        bottom_points = self.calc_ellipse_points(self.point_f, self.point_e, self.point_d, self.COL_COUNT)
    ...
    def get_ellipse_params(self, left, top, right):
        center = (left + right) / 2
        center_point = tuple(int(np.round(x)) for x in center.tolist())
...
if __name__ == "__main__":
    ...
    imcv = cv2.imread("reshape1.jpg", cv2.IMREAD_UNCHANGED)
